...
function UGCVideos() {
    ...
    useEffect(() => {
        const q = query(collection(db, 'Hussein'))
    ...
    const toggleComplete = async (script) => {
        await updateDoc(doc(db, 'Hussein', script.id), {
            completed: !script.completed
        })
    }

    const deleteScript = async (id) => {
        await deleteDoc(doc(db, 'Hussein', id));
    ...
    const moveToUGC = async (id) => {
        const sourceDocRef = (doc(db, 'Hussein', id));
                    ...
                    <div className='flex flex-col sm:flex-row w-full items-start justify-between '>
                        <div className='gap-y-4 w-full sm:gap-y-8 flex flex-col'>
                            <div className='flex justify-between items-center'>
                                <h4 className='text-md flex items-center gap-x-2 text-[#B4B4B4]'><Link to='/'>DASHBOARD</Link> <ArrowRight2 className='w-4 sm:w-max' size="19" color="#B4B4B4" /> <Link to='/ad-launch/hussein'>HUSSEIN</Link></h4>
                            </div>

                            <div className='flex pb-10 justify-between items-center'>
                                <h1 className='text-3xl sm:text-4xl text-[#dbdbdb] font-normal'>Hussein</h1>
                            </div>
                        </div>
